...
import { Receipt, Search, ChevronLeft, Download, Eye } from 'lucide-react';
...
import { fetchGroupTransactions } from '@/lib/api';
...
export function GroupTransactionsPage({ groupId }: { groupId?: string }) {
  ...
  useEffect(() => {
    if (!groupId) return;
    fetchGroupTransactions(groupId).then((t) => { setTransactions(t); setLoading(false); });
  }, [groupId]);
  ...
  const filtered = useMemo(() => transactions.filter((c) => {
    ...
    if (search) {
      ...
      return c.paymentReference.toLowerCase().includes(q) || c.contributorName.toLowerCase().includes(q) || c.transactionId.toLowerCase().includes(q);
    }
    return true;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display font-extrabold text-2xl sm:text-3xl text-ink-900">Transactions</h1>
          <p className="text-sm text-ink-500 mt-1">{filtered.length} transactions · {formatKES(totalAmount)} total</p>
        </div>
        <Button variant="outline" leftIcon={<Download className="w-4 h-4" />} onClick={() => toast('success', 'Export started.')}>Export CSV</Button>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total', value: formatKES(totalAmount) },
      ...
      <Card padding="md">
        <div className="flex flex-col sm:flex-row gap-3">
          <Input placeholder="Search by reference, name, or ID..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} leftIcon={<Search className="w-4 h-4" />} className="flex-1" />
      ...
      <Card padding="none" className="overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={<Receipt className="w-8 h-8" />} title="No transactions found" description="Try adjusting your filters." />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-ink-100 bg-ink-50/50">
                  {['Date', 'Contributor', 'Type', 'Amount', 'Fee', 'Reference', 'Method', 'Status'].map((h) => (
                  ...
                  <tr key={c.id} className="hover:bg-ink-50/50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        {c.contributorKind === 'registered' && <Avatar name={c.contributorName || '?'} color="bg-brand-500" size="sm" />}
                        <span className="text-sm font-medium text-ink-900">{c.contributorName || 'Anonymous'}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4"><span className="text-sm text-ink-600">{paymentMethodLabel[c.paymentMethod] || c.paymentMethod}</span></td>
